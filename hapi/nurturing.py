...
class NurturingClient(BaseClient):
    ...
    def get_campaigns(self, **options):
        return self.call('campaigns', **options)
    
    def get_leads(self, campaign_guid, **options):
        return self.call('campaign/%s/list' % campaign_guid, **options)
    
    def get_history(self, lead_guid, **options):
        return self.call('lead/%s' % lead_guid, **options)
    
    def enroll_lead(self, campaign_guid, lead_guid, **options):
        return self.call('campaign/%s/add' % campaign_guid, data=lead_guid, method='POST', **options)
    
    def unenroll_lead(self, campaign_guid, lead_guid, **options):
        return self.call('campaign/%s/remove' % campaign_guid, data=lead_guid, method='POST', **options)
